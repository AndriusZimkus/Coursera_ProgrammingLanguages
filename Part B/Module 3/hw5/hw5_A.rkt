...
(define (mupllist->racketlist ml)
  (cond [(aunit? ml) '()]
        ...
        [else (error (format "Invalid MUPL list: ~v" ml))]))

;; Problem 2

(define (extend-env env name var)
  (cons (cons name var) env))  ;; add a new pair (name . var) to the front
          ...
          (let ([funexp (eval-under-env (call-funexp e) env)]
              ...
              (if (closure? funexp)
                  (let* ([cenv (closure-env funexp)]
                         ...
                         [new-env (if (equal? name #f)
                                      (extend-env cenv (fun-formal fun) actual)   ;; No name, so bind just the parameters name with the actual parameters
                                      (extend-env (extend-env cenv name funexp) (fun-formal fun) actual))]) ;; extend with function name if not #f
                      (eval-under-env (fun-body fun) new-env))
                  (error (format "Not a function: ~v" funexp))))]
        [(mlet? e)
...
(define (mlet* lstlst e2)
  (if (null? lstlst)
      e2
      (let* ([this-pair (car lstlst)]
             [name (car this-pair)]
             [expr (cdr this-pair)])
          (mlet name expr (mlet* (cdr lstlst) e2)))))
