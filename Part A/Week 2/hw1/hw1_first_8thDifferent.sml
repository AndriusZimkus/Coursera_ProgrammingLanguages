...
fun number_before_reaching_sum (sum : int, listOfIntegers: int list) =
    let		    
	fun number_before_reaching_sum_helper(listOfIntegersForHelper: int list, previousSum : int, whichNumber : int) =
	    let
		val currentNumber = hd(listOfIntegersForHelper)
		val currentSum = previousSum+currentNumber
						
	    in
		if currentSum >= sum
		then whichNumber
		else number_before_reaching_sum_helper(tl(listOfIntegersForHelper),currentSum,whichNumber+1)
	    end   
					
    in
	number_before_reaching_sum_helper(listOfIntegers,0,0)
    end
