...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(@x1, @x2) # the segments are on a vertical line
      if @y1 < seg.y1 # let segment a start at or below start of segment b
        a = self
        b = seg
      else
        a = seg
      ...
      if @x1 < seg.x1 # let segment a start at or to the left of start of segment b
        a = self
        b = seg
      else
        a = seg
...
class Let < GeometryExpression
  ...
  def eval_prog env
    @e2.eval_prog([[@s, @e1.eval_prog(env)]] + Marshal.load(Marshal.dump(env)))
  end
  def preprocess_prog
